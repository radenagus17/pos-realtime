...
export default function FormMenu<T extends FieldValues>({
  ...
  return (
    <DialogContent className="sm:max-w-[425px] max-h-[90vh] px-0">
      <Form {...form}>
        <DialogHeader className="px-4">
          <DialogTitle>{type} Menu</DialogTitle>
          <DialogDescription>
            {type === "Create" ? "Create new menu" : "Make changes menu here"}
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={onSubmit}>
          <div className="space-y-4 max-h-[50vh] no-scrollbar overflow-y-auto px-4">
            <FormInput
              form={form}
              name={"name" as Path<T>}
              label="Name"
